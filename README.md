...
### 시나리오 만들기 → 띄우기 워크플로우
1. 게임 화면 상단 **📜 시나리오** → 라이브러리(`scenarios/index.html`)로 이동.
2. **＋ 새 시나리오 제작** → 카드 형식으로 장면(제목·본문·이미지)을 작성.
3. **⬇ 추출** 버튼으로 `이름.json` 파일을 내려받음.
4. 내려받은 파일을 저장소의 **`scenarios/`** 폴더에 넣음.
5. **`scenarios/manifest.json`** 배열에 그 파일명을 추가:
   ```json
   ["example-scenario.json", "내시나리오.json"]
   ```
6. 사이트를 새로고침하면, 게임 마스터 패널의 **시나리오 선택**에 나타납니다.
7. 장면 카드의 **띄우기 ▶** 를 누르면 제목·본문·이미지가 모든 접속자에게 실시간으로 표시됩니다.

> 매니페스트 등록이 번거로우면, 마스터 패널의 **“파일에서 불러오기”** 로 추출한 JSON을 그 자리에서 바로 불러와 쓸 수도 있습니다(폴더/새로고침 불필요).
> 로컬에서 `file://` 로 열면 폴더 목록 로드가 막힐 수 있으니, 시나리오 목록 확인은 GitHub Pages(http) 환경에서 하세요.

### 턴 시스템
- 게임 시작 후 마스터 패널의 **턴 시작/처음 · 이전 · 다음 · 종료** 로 진행합니다.
- 현재 턴인 캐릭터 카드가 강조되고, 상단에 턴 배너가 뜨며, 턴이 바뀔 때마다 화면 중앙에 **“NO.0X 이름 의 턴”** 안내가 잠깐 표시됩니다.
- 플레이어(관전자)에게도 동일하게 실시간으로 보입니다.

### 추가된 주사위 애니메이션
- 능력치뿐 아니라 **소속 굴리기**와 **전체 판정 실행**에도 같은 굴림 애니메이션 + 소리가 적용됩니다.
